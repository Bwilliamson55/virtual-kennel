<template>
  <q-card class="q-ma-md">
    <q-toolbar>
      <span class="text-h6">Hounds</span>
      <q-space />
      <q-btn
        flat
        round
...
const updateHounds = () => {
  ...
  houndsStore
    .fetchHounds({
      ...
      sort: [
        `${houndsStore.pagination.descending ? "-" : ""}${
          houndsStore.pagination.sortBy
        }`
      ],
      limit: houndsStore.pagination.rowsPerPage,
      offset:
        (houndsStore.pagination.page - 1) * houndsStore.pagination.rowsPerPage,
      search: filterQuery.value
    })
    .then(() => {
      hounds.value = houndsStore.hounds;
      loading.value = false;
    });
};
